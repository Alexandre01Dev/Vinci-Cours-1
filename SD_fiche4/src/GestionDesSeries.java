...
public class GestionDesSeries {
	...
	private static MonScanner scanner = new MonScanner("SD_fiche4/inputGestionDesSeries.txt");
	...
	private static void rechercherSerie() {
		...
		System.out.println(classe.numeroSerie(nom));
	}
	...
	private static void afficherTailleSeries() {
		//TODO
		System.out.println("Taille des series -> ");
		int total = 0;
		for (int i = 1; i <= classe.nombreSeries(); i++) {
			int nb = classe.getSerieEtudiants(i).nombreEtudiants();
			System.out.println("  série "+i +" -> "+ nb +" étudiants");
		}
	...
	private static void afficherTout() {
		...
		for (int i = 1; i <= classe.nombreSeries(); i++) {
			System.out.println("Serie "+ i +": "+ classe.getSerieEtudiants(i).toString());
		}
	}
