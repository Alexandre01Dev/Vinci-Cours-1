...
public class Vol {
	...
	public boolean estUnTour(){
		...
		//TODO// pensez a utiliser la methode equals() de la classe Coordonnee
	...
	public Coordonnees niemeLieu(int n){	
		...
		//TODO
		if(n < 0 || n > tableCoordonnees.length-1){
			return null;
		}
	...
	public Coordonnees lieuLePlusAuSud() {
		Coordonnees select = null;
		for (Coordonnees coordonnees : tableCoordonnees) {
			if(select == null){
				select = coordonnees;
				continue;
			}
			// si nouvelle latitude plus petite que celle qui est séléctionnée
			if(coordonnees.getLatitude() < select.getLatitude()){
				select = coordonnees;
			}
		}
		return select;
	...
	public Coordonnees lieuLePlusEloigne(){
		...
		int index = 0;
		Coordonnees firstPoint = tableCoordonnees[0];
		for (Coordonnees coordonnees : tableCoordonnees) {
			if(select == null){
				select = coordonnees;
				index++;
			...
			if((distance = coordonnees.distance(firstPoint)) > currentDistance){
				System.out.println("Selection du vol "+ index+" -> "+coordonnees +" -> "+ distance);
				currentDistance = distance;
				select = coordonnees;
				index++;
				continue;
			}
			System.out.println("Vol "+ index + " ignoré: distance -> "+ coordonnees.distance(select) + " et "+ coordonnees);
			index++;
		}
	...
	public double distance() {
		...
		// la distance est un reel (double)
		double total = 0;
		Coordonnees last = null;
		for (Coordonnees coordonnees : tableCoordonnees) {
			if(last == null){
				last = coordonnees;
			}

			total += last.distance(coordonnees);
			last = coordonnees;
		}
		return total;
	}
	...
	public boolean aSurvoleUnMemeLieu() {
		...
		for (Coordonnees iter1 : tableCoordonnees) {
			for (Coordonnees iter2 : tableCoordonnees) {
				if(iter1 == iter2){
					continue;
				}
